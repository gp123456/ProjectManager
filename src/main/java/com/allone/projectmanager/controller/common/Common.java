...
public class Common {
    ...
    public String createSearchVessel(ProjectManagerService srvProjectManager) {
    ...
    public void setVirtualProjectBillInfo(Long itemId, Model model, ProjectManagerService srvProjectManager) {
        ...
        if (item != null) {
            mapProjectBillItems.put(itemId, new ProjectBillItem().getBuilder().setAvailable(0).setQuantity(item.
                                    getQuantity()).setPrice(item.getPrice()).setItem(item).build());
        } else {
            item = srvProjectManager.getDaoItem().add(new Item().getBuilder().setId(itemId).build());
            mapProjectBillItems.put(itemId, new ProjectBillItem().getBuilder().setAvailable(0).setQuantity(0).setPrice(
                                    BigDecimal.ZERO).setItem(item).build());
        }
    }

    public void editVirtualProjectBillInfo(ProjectBillItem pbi) {
        mapProjectBillItems.put(pbi.getItem().getId(), pbi);
    }

    public void editVirtualProjectBillInfo(ProjectBill pb) {
        for (ProjectBillItem pbi : mapProjectBillItems.values()) {
            pbi.setId(null);
            pbi.setProjectBill(pb);
        }
    }
